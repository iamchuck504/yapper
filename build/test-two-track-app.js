...
// (needs the engine installed and the fixture: node build/make-fixtures.js)
...
app.whenReady().then(async () => {
  try {
    ...
    if (!fs.existsSync(SRC)) { say('skip  fixture missing: node build/make-fixtures.js'); app.exit(0); return; }
    ...
    const labelled = lines.filter(l => / (Me|Them): /.test(l));
    ...
    check('Them speaks only between 18 and 42 s',
      lines.filter(l => l.includes(' Them: ')).every(l => secOf(l) >= 18 && secOf(l) <= 42));
    check('both sides made it in',
      lines.some(l => l.includes(' Me: ')) && lines.some(l => l.includes(' Them: ')));
