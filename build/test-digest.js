...
const standup = meeting('2026-07-29_0900', {
  title: 'Standup', participants: ['Chuck', 'Ninfa'],
  sections: [
    ...
    { heading: 'Action items [01:00]', body: '- Ninfa: update the changelog' }
  ]
});
...
const ITEMS = [
  { text: 'update the changelog', owner: 'Ninfa', due: '', priority: 'normal', done: false, folder: standup.folder, meeting: 'Standup', sources: [standup.folder] },
...
check('la gente, por cuántas veces apareció',
  facts.people.map(p => `${p.name}:${p.count}`), ['Chuck:2', 'Carlos:1', 'Ninfa:1']);
...
ok('lleva los títulos y las fechas',
  /=== Standup \| 2026-07-29 \| Chuck, Ninfa ===/.test(input.text), input.text.slice(0, 200));
