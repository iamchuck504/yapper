...
if (-not (Test-Path $sevenZa)) { Write-Host "FAIL sin 7za en $sevenZa"; exit 1 }

$archives = Get-ChildItem $cache -Filter '*.7z'
if (-not $archives) { Write-Host "FAIL no hay archivos .7z en $cache"; exit 1 }
...
foreach ($a in $archives) {
    ...
    Write-Host ("  exit {0} (2 = solo fallaron los symlinks de macOS)" -f $LASTEXITCODE)
    ...
    else { Write-Host "  FAIL sin rcedit en $slot" }
}
Write-Host "listo"
