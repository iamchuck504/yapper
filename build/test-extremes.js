...
app.whenReady().then(async () => {
  ...
  say('--- 1. the clock in long meetings ---');
  ...
  check('just short of the hour', stamps[3] === '59:59', stamps[3]);
  check('crossing the hour adds the field', stamps[4] === '01:00:00', stamps[4]);
  ...
  check('with no stamp it invents none', parsed[3].at === '' && parsed[3].title === 'No stamp here',
  ...
  check('the header repairs itself in a file like that',
  ...
  const tr = await within(
    ...
    'transcribir 75 min', 600000).catch(e => 'hung:' + e.message);
  ...
  check('transcribes a meeting over an hour long', typeof tr === 'number' && tr > 1000, String(tr).slice(0, 120));
  check('memory does not blow up', grew < 400, `grew ${grew.toFixed(0)} MB`);

  const text = fs.readFileSync(path.join(long, 'transcript.txt'), 'utf8');
  const last = text.trim().split('\n').pop();
  say(`  last line: ${last.slice(0, 70)}`);
  check('the stamps go past an hour', /^\[0[1-9]:/.test(last), last.slice(0, 40));
  check('the stamps are in order',
    (() => {
  ...
  const r4 = await within(
    ...
    'transcribe an empty wav', 60000).catch(e => 'hung:' + e.message);
  say(`  empty wav: ${String(r4).slice(0, 90)}`);
  check('an empty wav neither hangs nor blows up ugly', !String(r4).startsWith('hung'), String(r4));
  ...
  say(`  ${rows} rows in ${Date.now() - t5} ms`);
  check('lists hundreds of meetings', rows >= 300, `${rows} rows`);
  check('and does not take forever', Date.now() - t5 < 8000, `${Date.now() - t5} ms`);
  ...
  check('and finds the meeting', found >= 1 && found <= 3, `${found} results`);
  ...
  if (started) {
    ...
    check('the live buffer does not grow without bound', grewLive < 60, `grew ${grewLive.toFixed(0)} MB`);
    check('it catches up and confirms the current window', confirmed >= 20, `${confirmed} words`);
    check('with no errors along the way', errors === 0, `${errors} errors`);
    await live.stop();
    await engine.stop();
