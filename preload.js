...
contextBridge.exposeInMainWorld('yapper', {
  ...
  cancelNotes: folder => invoke('cancel-notes', folder),
  ...
  setSpeakerMap: (folder, map) => invoke('set-speaker-map', folder, map),
  ...
  updateReminders: (ids, fields) => invoke('update-reminders', ids, fields),
  renameMeeting: (folder, title) => invoke('rename-meeting', folder, title),
  onUiCommand: cb => ipcRenderer.on('ui-command', (_e, name) => cb(name)),
