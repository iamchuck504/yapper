...
- **Local speaker detection on macOS 14+.** The full transcript keeps stable local voice labels so different viewpoints are not mixed, and a meeting panel lets you match them to real participant names. Generated notes preserve real names and every explicitly named action owner; only unknown voices become neutral prose, so they never read like “Speaker 1 said…” or “Speaker 2 decided…”. Failure falls back to the reliable two-track transcript without blocking the meeting.
